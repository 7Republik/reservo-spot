    ...
    >
      ...
      <div className="relative z-10 p-1 sm:p-1.5 md:p-2 flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-0.5 sm:mb-1">
          ...
          {/* Icono de estado */}
          {reserved && (
            <div className="w-4 h-4 sm:w-6 sm:h-6 rounded-full bg-emerald-500 flex items-center justify-center shadow-sm">
        ...
        <div className="mt-auto">
          ...
          {loadingSpots && !reserved && !isPast && (
            <div className="flex items-center gap-0.5 sm:gap-1 animate-pulse">
              <div className="w-1 h-1 sm:w-2 sm:h-2 rounded-full bg-gray-300" />
          ...
          {!loadingSpots && !isPast && !reserved && available > 0 && (
            <div className="flex items-center gap-0.5 sm:gap-1">
              <div className="w-1 h-1 sm:w-2 sm:h-2 rounded-full bg-emerald-500" aria-hidden="true" />
              <span className="text-[0.55rem] sm:text-xs font-medium text-emerald-700">
                {available}
              </span>
            </div>
          )}
          {!loadingSpots && available === 0 && !isPast && !reserved && (
            <div className="flex items-center gap-0.5 sm:gap-1">
              <div className="w-1 h-1 sm:w-2 sm:h-2 rounded-full bg-red-500" aria-hidden="true" />
              <span className="text-[0.55rem] sm:text-xs font-medium text-red-700">
                No
              </span>
            </div>
          )}
